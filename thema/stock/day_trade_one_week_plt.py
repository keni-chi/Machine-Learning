...
if not os.path.exists(SAMPLE_DIR):
    ...
    os.makedirs(f'day_trade_{local_rundate}/one_week')


def one_day_plt():
    df_acc = pd.read_csv(f'simulate_{local_rundate}/df_acc.csv', index_col=0, encoding='shift-jis')
    ...
    for code in code_list:
        ...
        df['mean15'] = df['Close_ratio'].rolling(15).mean()
        df['mean20'] = df['Close_ratio'].rolling(20).mean()

        df['mean60'] = df['Close_ratio'].rolling(60).mean()
        df['mean120'] = df['Close_ratio'].rolling(120).mean()
        df['mean240'] = df['Close_ratio'].rolling(240).mean()
        ...
        # ax.plot(df.index, df['mean15'])
        # ax.plot(df.index, df['mean20'])
        # ax.plot(df.index, df['mean60'])
        # ax.plot(df.index, df['mean120'])
        # ax.plot(df.index, df['mean240'])
        ax2 = ax.twinx()
        ...
        # ax.set_ylabel('y')
        ax.legend()
        plt.savefig(f'day_trade_{local_rundate}/one_week/{code}.png')
        plt.close()

        df.to_csv(f'day_trade_{local_rundate}/one_week/{code}.csv')
