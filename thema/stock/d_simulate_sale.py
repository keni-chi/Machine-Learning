...
class Sale():
    ...
    def run(self):
        ...
        # startのtimestampでループ
        for d in df_start['timestamp'].tolist():
            # Openの価格を取得。
            df_raw = self.df_raw.reset_index()
            df_open = df_raw[df_raw['timestamp'] ==d]
            open_v = df_open['Open'].values[0]
            
            # upは1.1倍、downは0.95倍、で求める。
            up_v = d_simulate_conf.rikaku_ratio * open_v
            down_v = d_simulate_conf.songiri_ratio * open_v
            ...
            if len(df_up)==0:
                ...
                up_date = df_up['timestamp'].dt.strftime('%Y-%m-%d %H:%M:%S').values[0]
            # print('A01')
            # print(up_v)
            # print(df_up)
            # print(up_date)
            ...
            if len(df_down)==0:
                ...
                down_date = df_down['timestamp'].dt.strftime('%Y-%m-%d %H:%M:%S').values[0]
            # print('A02')
            # print(down_v)
            # print(df_down)
            # print(down_date)
            ...
            if up_date != '該当なし':
                up_date_t = time.strptime(up_date, "%Y-%m-%d %H:%M:%S")
            if down_date != '該当なし':
                down_date_t = time.strptime(down_date, "%Y-%m-%d %H:%M:%S")
        ...
        print('A001')
        print(df_soneki)
        df_soneki['date1'] = pd.to_datetime(df_soneki['timestamp'])
