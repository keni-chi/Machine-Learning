...
sub_folder = f'usa_kmeans{usa_simulate_conf.RUNDATE}'
SAMPLE_DIR = f'usa_simple_reg_{usa_simulate_conf.RUNDATE}'
...
def fit_predict(df):
    ...
    df_dataset.to_csv(f'usa_simple_reg_{usa_simulate_conf.RUNDATE}/df_dataset.csv', encoding='shift-jis')
    ...
    df_dataset_up.to_csv(f'usa_simple_reg_{usa_simulate_conf.RUNDATE}/df_dataset_up.csv', encoding='shift-jis')
...
def simple_reg(df, interval):
    ...
    # 出力
    df_result.to_csv(f'usa_simple_reg_{usa_simulate_conf.RUNDATE}/df_simple_reg.csv', encoding='shift-jis')

    return df_result


def plot_scatter(X, y, y_pred, model_lr, mape_score, interval, code):
    df_dataset_up = pd.read_csv(f'usa_simple_reg_{usa_simulate_conf.RUNDATE}/df_dataset_up.csv', index_col=0, encoding='shift-jis')
    ...
    if str(code) in up_code_list:
        ...
        ax.set_ylabel('y')
        ax.legend()
        # plt.savefig(f'simple_reg_{usa_simulate_conf.RUNDATE}/png/{interval}_{str(code)}.png')
