...
from simulate_detect import Detect
from simulate_buy import Buy
from simulate_sale import Sale


root_folder = f'simulate_{simulate_conf.RUNDATE}'
...
def calc_run(code_list):
    for code in code_list:  #[1852]:
        ...
        # 生データ読み込み
        df = pd.read_csv(f'./data-yahoo-{simulate_conf.RUNDATE}/stock_{str(code)}.csv', index_col=0, encoding='shift-jis')
        # 5844でnanエラーの対応
        df = df.interpolate()
...
def aggregate_simulation(code_list, df):
    df_agg = pd.DataFrame()
    for code in code_list:  #[1852]:
        df_weekday_meta_up = pd.read_csv(f'./simulate_{simulate_conf.RUNDATE}/{str(code)}/df_total_soneki.csv', index_col=0, encoding='shift-jis')
    ...
    # 出力
    df_agg.to_csv(f'simulate_{simulate_conf.RUNDATE}/df_agg.csv', encoding='shift-jis')
    time.sleep(2)


def aggregate_accumulation():
    path_list = glob.glob('./simulate_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]')
    ...
    df_agg = pd.read_csv(f'simulate_{simulate_conf.RUNDATE}/df_agg.csv', index_col=0, encoding='shift-jis')
...
def main():
    # codeリストを取得
    df = pd.read_csv(f'./static_month_{simulate_conf.RUNDATE}/df_weekday_meta_up.csv', index_col=0, encoding='shift-jis')
    # df = pd.read_csv(f'./static_month_{simulate_conf.RUNDATE}/df_weekday_meta.csv', index_col=0, encoding='shift-jis')
