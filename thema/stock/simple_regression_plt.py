...
sub_folder = f'kmeans{config.RUNDATE}'
SAMPLE_DIR = f'simple_reg_{config.RUNDATE}'
...
def fit_predict(df):
    ...
    df_dataset.to_csv(f'simple_reg_{config.RUNDATE}/df_dataset.csv', encoding='shift-jis')
    ...
    df_dataset_up.to_csv(f'simple_reg_{config.RUNDATE}/df_dataset_up.csv', encoding='shift-jis')
...
def simple_reg(df, interval):
    ...
    # 出力
    df_result.to_csv(f'simple_reg_{config.RUNDATE}/df_simple_reg.csv', encoding='shift-jis')
...
def plot_scatter(X, y, y_pred, model_lr, mape_score, interval, code):
    ...
    if str(code) in up_code_list:
        ...
        ax.set_ylabel('y')
        ax.legend()
        plt.savefig(f'simple_reg_{config.RUNDATE}/png/{interval}_{str(code)}.png')
        plt.close()
